...
const nodeA = grid.getNodeId('B4_F26');
console.log('Probe Node B4_F26 Node ID:', nodeA);

// Print Node IDs of Power Binding Posts & Rails
console.log('BINDING_Va Node ID:', grid.getNodeId('BINDING_Va'));
console.log('BINDING_Vc Node ID:', grid.getNodeId('BINDING_Vc'));
console.log('VCC_TOP1_3 Node ID:', grid.getNodeId('VCC_TOP1_3'));
console.log('VCC_TOP2_22 Node ID:', grid.getNodeId('VCC_TOP2_22'));
console.log('B4_A27 Node ID:', grid.getNodeId('B4_A27'));
console.log('B3_VCC_R_27 Node ID:', grid.getNodeId('B3_VCC_R_27'));

const samples = [];
for (let i = 0; i < 3000; i++) {
    const res = solver.solveStep(activeComps, 0.000005);
    samples.push(res.get(nodeA) || 0);
}

const last100 = samples.slice(-100);
console.log('CH A Min:', Math.min(...last100).toFixed(2), 'Max:', Math.max(...last100).toFixed(2), 'Vpp:', (Math.max(...last100) - Math.min(...last100)).toFixed(2));
console.log('Sample sequence (first 25 of last 100):');
console.log(last100.slice(0, 25).map(x => x.toFixed(2)).join(', '));
