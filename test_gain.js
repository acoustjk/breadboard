...
const comps = [
    ...
    new Wire('WIRE_64', 'B2_VCC_R_6', 'B2_A6', '#ef4444'),
...
for (const ratio of [0.15, 0.20, 0.25, 0.28, 0.32, 0.40]) {
    comps.find(c => c.id === 'RESISTOR_CATALOG_74').ratio = ratio;
    const samples = [];
    for (let i = 0; i < 4000; i++) {
        const res = solver.solveStep(activeComps, 0.000005);
        if (i > 3500) samples.push(res.get(nodeA) || 0);
    }
    const min = Math.min(...samples);
    const max = Math.max(...samples);
    console.log(`Ratio ${(ratio*100).toFixed(0)}% (${(ratio*1000).toFixed(0)}kΩ) -> Vmin: ${min.toFixed(2)}V, Vmax: ${max.toFixed(2)}V, Vpp: ${(max-min).toFixed(2)}V`);
}
