...
const nodeTP2 = grid.getNodeId(pinTP2);
const nodeTP3 = grid.getNodeId(pinTP3);
...
for (let i = 0; i < 5000; i++) {
    ...
    samplesTP3.push(res.get(nodeTP3) || 0);
}

const checkWave = (name, samples) => {
    const last200 = samples.slice(-200);
    const minV = Math.min(...last200);
    const maxV = Math.max(...last200);
    const vpp = maxV - minV;
    console.log(`=== ${name} ===`);
    console.log(`Min: ${minV.toFixed(2)}V, Max: ${maxV.toFixed(2)}V, Vpp: ${vpp.toFixed(2)}V`);
    console.log(`Sample sequence (first 15 of last 200):`);
    console.log(last200.slice(0, 15).map(x => x.toFixed(2)).join(', '));
};

checkWave('TP1 (CH A - Sine Wave)', samplesTP1);
checkWave('TP2 (CH B - Square Wave)', samplesTP2);
checkWave('TP3 (CH C - PNM Burst Pulse)', samplesTP3);
